...
object_url = 'http://172.26.21.15:8124/jetbot_object_detection.txt'
object_path = './jetbot_object_detection.txt'

jetbot_coord_url = 'http://172.26.21.15:8124/webpage/area_coord.txt'
jetbot_coord_path = './area_coord.txt'

send_bombot_url = 'http://172.26.21.15:8124/webpage/send_bombot.txt'
send_bombot_path = './send_bombot.txt'
...
class Multipoint_navigation:
    def __init__(self):
        ...
        # Publish tts
        self.pub_tts = rospy.Publisher('/speak_object', String, queue_size = 10)
        # 이미지 보내는 subcriber
        ...
        self.boundary = {
            ...
            "bottom_left": (-0.05, -0.115),
            "bottom_right": (-0.0486, -0.981),
            "top_right": (-0.894, -0.997)
        }
        ...
        self.download_text(jetbot_coord_url, jetbot_coord_path)
        self.download_text(send_bombot_url, send_bombot_path)

        cnt = 0
        self.rotation_count = 0
        self.detected = False    
        boundary_file_path = "./area_coord.txt"  # 실제 파일 경로로 수정 필요
        self.load_boundary_from_text(boundary_file_path)        
...
#                 print(x, y)
                ...
                self.set_target_point(float(self.center_y), float(self.center_x))
...
#                 self.take_photo_scotty()
                self.take_photo_object()
                rospy.sleep(4)  ## taks3는 2초, scotty 는 1초
...
#                 print('감지결과:', detected)          
    ...
    def load_boundary_from_text(self, file_path):
        print(file_path)
        points = []
        try:
            with open(file_path, 'r') as f:
                for line in f:
                    ...
                    points.append((x, y))
            if len(points) == 4:  # 네 개의 좌표가 제대로 로드되었는지 확인
                self.boundary = {
                    "top_left": points[0],
            ...
            print("Failed to load boundary from "+file_path+": "+str(e))        
    ...
    def calculate_object_position(self):
        ...
        # Assuming JetBot's initial position is at (0,0)
        jetbot_x, jetbot_y = self.jetbot_position_x, self.jetbot_position_y
        ...
        with open('object_position.txt', 'w') as file:
            file.write("Object's position: ({:.2f}\n{:.2f})\n".format(object_x, object_y))        
...
#             self.chatter_subscriber.unregister()
    ...
    def take_photo_scotty(self):  # fale
        ...
        scotty_save_path = "/home/jetbot/catkin_ws/src/jetbot_pro/scripts/scotty_real.jpg"
#         self.calibration(image, scotty_save_path)
        

        cv2.imwrite(scotty_save_path, image)
...
#             self.scotty_position()
    ...
    def take_photo_object(self):
        # # 카메라를 중지하고 이미지를 읽은 후 다시 실행
        print("****** Take Photo Object *******")
        self.picture_cnt+=1
        if self.picture_cnt > 1:
            print("Already processed a picture, exiting method.")
            self.rotation_count+=1
        ...
        image = self.camera.value
#         image = self.cal_undistort(image)
        cv2.imwrite("/home/jetbot/catkin_ws/src/jetbot_pro/scripts/object_detection_jetbot/jetbot_object_detection.png", image)

        print("object_image: ", type(image), image.shape)
        self.download_text(object_url,object_path)
        # self.download_text(jetbot_coord_url, jetbot_coord_path)
        # self.download_text(send_bombot_url, send_bombot_path)
        result = self.check_object_in_file()
        print(result)
        if result == True and self.picture_cnt==1:
#             self.scotty_position()
            print('We got True')

            self.pub_tts.publish("bomb detected")
            print('yes')
    ...
    # def calibration(self, img, save_path):
    #         self.image_pub.publish(self.br.cv2_to_compressed_imgmsg(img))
    #         def callback_save(img):
    #             cal_img = self.br.compressed_imgmsg_to_cv2(img, "bgr8")
    #             cv2.imwrite(save_path, cal_img)
    #         rospy.Subscriber('imagesave', CompressedImage, callback_save)
    #         rospy.loginfo("Calibration finished and image saved")
    ...
    def area_coord_in_file(self):
        try:
            with open(jetbot_coord_path, "r") as file:
                content = file.read()
                if "Scotty" in content:
                    return True
                else:
            ...
            print("check area coord in file not found!")
            return False


    def check_object_in_file(self):
        try:
            with open("./jetbot_object_detection.txt", "r") as file:
                content = file.read()
                # if "tv" in content:
                if "True" in content:
                    return True
            ...
            print("check object in file not found!")
            return False
